...
def prune_invalid(tokens):
    ...
    for t in tokens:
        ...
        if len(t.access_token) and len(t.provider):
            ...
            if active:
                logging.debug('token %s belonging to uid %s was valid', t.access_token, t.user.id)
                valid.append(t)
            else:
                # try refresh
                try:
                    ...
                    logging.debug('token %s belonging to uid %s was revoked', t.access_token, t.user.id)
                    t.delete()
        else:
...
def _user_from_args(uid, kwargs):
    if kwargs.get('token', None):
        ...
        logging.debug('got user (id %s) from bearer token', user.id)
